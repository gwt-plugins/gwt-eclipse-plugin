...
public class SdkUtils {
  ...
  public abstract static class MaxSdkVersionComputer {
            ...
            if (currentMaxVersion == null) {
              ...
              try {
                ...
                CorePluginLog.logError(e,
                    "Could not compare '" + currentMaxVersion + "' to '" + version + "', ignoring this version");
              }
            }
    ...
    /**
     * Returns the {@link Sdk} that is bound to the {@link IJavaProject} or <code>null</code> if none.
     */
    public abstract Sdk doFindSdk(IJavaProject project) throws JavaModelException;
  }
  ...
  /**
   * In some cases, the 3rd component of the version string can contain both digits and non-digits (e.g., 2.1.0M1,
   * 2.1.0-m1); this is incompatible with what GPE expects for a SDK version string.
   */
  public static String cleanupVersion(String versionStr) {
    ...
    // Add 4th component
    return versionComponents[0] + '.' + versionComponents[1] + '.' + lastComponent.substring(0, nonDigitIndex) + '.'
        + lastComponent.substring(nonDigitIndex);
  }

  /**
   * Compares two version strings for order. Returns a negative integer, zero, or a positive integer if the first
   * argument is less than, equal to, or greater than the second.
   * <p>
   * This method handles version strings that include RC or MS, treating them as MS < RC < final version.
   *
   * @param version1
   * @param version2
  ...
  /**
   * Returns an {@link Sdk} with a matching installation path from the {@link SdkSet} or <code>null</code> if none could
   * be found.
   */
  public static <T extends Sdk> T findSdkForInstallationPath(SdkSet<T> sdkSet, IPath installationPath) {
    for (T sdk : sdkSet) {
  ...
  /**
   * Generates a name that is unique to a given {@link SdkSet} based on the name prefix.
   *
   * @param namePrefix
   *          name prefix for the generated name
   * @param sdks
   *          {@link SdkSet} for within which a unique name is needed
   *
   * @return name that is unique to a given {@link SdkSet} based on the name prefix
   */
  ...
  /**
   * Returns a negative numeric representation of the prerelease version string. This is negative so all prereleases are
   * less than the corresponding release.
   *
   * @param versionString
   *          must start with either "rc", "beta" or "ms" followed by a number that is less than 1000
   * @return a negative numeric representation such that r(rc1) < r(rc10) and r(ms__) < r(rc1)
   * @throws NumberFormatException
   */
  ...
  private static int parseVersionComponent(String[] versionComponents, int componentIndex)
    ...
    if (componentIndex < versionComponents.length) {
      ...
      if (component.length() > 0) {
        if (component.startsWith("rc") || component.startsWith("ms") || component.startsWith("beta")) {
  ...
  /**
   * Return a string where the modifier is separated from the version number by a '.' instead of a '-'. OSGI strings
   * want a '.' to separate the modifier from the rest of the version number.
   */
  private static String toOSGICompatibleVersionString(String versionString) {
    /*
     * Note this method does try to parse out the version number, to ensure that the replacement only takes place at the
