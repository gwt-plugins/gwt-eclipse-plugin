...
public class JpaProjectsListener implements CollectionChangeListener {
	...
	public static void start() {
		if (listener == null) {
			...
			JpaProjectManager manager = (JpaProjectManager) ResourcesPlugin
					.getWorkspace().getAdapter(JpaProjectManager.class);
			manager.addCollectionChangeListener(
	...
	public static void stop() {
		JpaProjectManager manager = (JpaProjectManager) ResourcesPlugin
				.getWorkspace().getAdapter(JpaProjectManager.class);
		manager.removeCollectionChangeListener(
				JpaProjectManager.JPA_PROJECTS_COLLECTION, listener);
		listener = null;
	}
	...
	public void itemsAdded(CollectionAddEvent event) {
		for (Object o : event.getItems()) {
			...
			if (GaeNature.isGaeProject(jpaProject.getProject())) {
				JpaFacetHelper.jobDisableDataNucleus(jpaProject
						.getJavaProject());
				JpaFacetHelper.jobUpdatePersistenceAndWebInf(
						jpaProject.getJavaProject(),
						new JpaFacetHelper.Updater());
			}
		}
	}
