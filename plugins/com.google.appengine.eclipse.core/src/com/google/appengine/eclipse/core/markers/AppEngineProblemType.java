...
public enum AppEngineProblemType implements IGdtProblemType {
  ...
  JAVA16_DEPRECATED(APP_ENGINE_OFFSET + 7, "Upgrade to Java 1.7",
      "App Engine requires Java 1.7 or later. ",
      GdtProblemSeverity.ERROR),

  // NOTE: This problem type should really be moved into the Cloud Endpoints plugin
  // The substring {0} should be the specific error message from the API generator
  CLOUD_ENDPOINTS_BUILD_PROBLEM(APP_ENGINE_OFFSET + 7, "Cloud Endpoints API generation failure",
      "There was a problem generating the API metadata for your Cloud Endpoints classes: {0}",
      GdtProblemSeverity.ERROR),
  ...
  REPACKAGED_IMPORTS_DISALLOWED(APP_ENGINE_OFFSET + 5, "Repackaged imports are not allowed",
      "Use of com.google.appengine.repackaged may result in your app breaking without warning.",
      GdtProblemSeverity.ERROR),

  /*
   * Commented out because this problem has been superseded by the JAVA16_DEPRECATED problem. We
   * leave it here so that we don't re-use the same problem ID.
   */
  // JAVA15_DEPRECATED(APP_ENGINE_OFFSET + 4, "Upgrade to Java 1.5",
  // "Future versions of the Dev App Server will require Java 1.5 or later",
  // GdtProblemSeverity.WARNING),

  JSP_WITHOUT_JDK(APP_ENGINE_OFFSET + 3, "JSP files in project without JDK",
      "Your project must be configured to use a JDK in order to use JSPs", GdtProblemSeverity.ERROR),
  ...
  UNSUPPORTED_JRE_TYPE(APP_ENGINE_OFFSET + 1, "Use of non-whitelisted JRE type",
  ...
  private AppEngineProblemType(int problemId, String description, String message,
