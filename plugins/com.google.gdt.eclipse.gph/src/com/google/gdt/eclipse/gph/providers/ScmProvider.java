...
public final class ScmProvider implements Comparable<ScmProvider> {
  ...
  ScmProvider(IConfigurationElement element) {
    ...
    providerDescription =
        element.getAttribute(ScmProviderRegistry.ExtensionId.SCM_PROVIDER_DESCRIPTION);

    String imagePath = element.getAttribute(ScmProviderRegistry.ExtensionId.SCM_PROVIDER_IMAGE);

    if (imagePath != null) {
      providerImageDescriptor =
          AbstractUIPlugin.imageDescriptorFromPlugin(element.getContributor().getName(), imagePath);
    }

    try {
      priority =
          Integer.parseInt(element.getAttribute(ScmProviderRegistry.ExtensionId.SCM_PRIORITY));
  ...
  /**
   * Returns the checkout provider for this SCMProvider. This is used in the project import wizard.
   */
  public ICheckoutProvider getCheckoutProvider() {
    if (checkoutProvider == null) {
      try {
        checkoutProvider =
            (ICheckoutProvider) element
                .createExecutableExtension(ExtensionId.CHECKOUT_PROVIDER_TYPE);
      } catch (Throwable t) {
  ...
  /**
   * Returns the SCM provider priority. This is used to decide between two providers for the same
   * SCM type. Lower priority wins.
   *
