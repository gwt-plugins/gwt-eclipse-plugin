...
/**
 * Auto edit strategy for JSNI blocks. Auto-indents each line based on its
 * position relative to blocks (e.g., { + \n adds adds another level of
 * indentation and } + \n resets the indentation to match the { it's closing).
 * Based on JavaAutoIndentStrategy from the Eclipse Java editor example plug-in.
 */
@SuppressWarnings("restriction")
public class JsniAutoEditStrategy extends DefaultIndentLineAutoEditStrategy {

  private final Map<?, ?> prefs;

  public JsniAutoEditStrategy(Map<?, ?> prefs) {
    this.prefs = prefs;
  }
  ...
  private int getBracketCount(IDocument document, int start, int end,
    ...
    while (begin < end) {
      ...
      switch (curr) {
        case '/':
          // If this comment is actually the start of the JSNI block, don't
          // treat it as a comment, and continue normal bracket counting
          if (begin < end + 2) {
            if (document.get(begin, 3).equals("*-{")) {
              continue;
            }
          }
          if (begin < end) {
   ...
   * @throws BadLocationException in case <code>position</code> is invalid in
   *           the document
   */
