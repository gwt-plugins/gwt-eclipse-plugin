...
/*
 * The E36 version is based off of this.
 */
/**
  ...
  private static class InstallUpdateHandler extends InstallNewSoftwareHandler {
    ...
    protected void doExecute(String profileId,
      ...
      try {
        ...
        InstallWizard wizard = new InstallWizard(Policy.getDefault(),
            profileId, null, null, manager);
    ...
    private URI findGpeRepoUri() throws Exception {
      ...
      String gpeUpdateSiteHostLower = gpeUpdateSiteUri.getHost().toLowerCase();

      // If the user has the search domains set to e.g. google.com, allow
      // http://dl/eclipse/plugin/3.5 to match
      // http://dl.google.com/eclipse/plugin/3.5
      URI[] repos = ProvisioningUtil.getMetadataRepositories(0);
      for (URI repo : repos) {
        if (repo != null && repo.getHost() != null
            && gpeUpdateSiteHostLower.startsWith(repo.getHost().toLowerCase())
     ...
     * {@link Policy} to the {@link InstallWizard}, but the P2 implementation
     * gives higher priority to the user settings. So, we save the user
     * settings, set them to values suitable for updating, show the dialog, and
